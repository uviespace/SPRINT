...
function manageData() {
	$.ajax({
		dataType: 'json',
		url: url+'api/getData_tab-enumeration.php',
		data: {page:page}
	}).done(function(data){
...
$(".crud-submit").click(function(e){
    ...
    var schema = $("#create-item").find("textarea[name='schema']").val();

    if(id != '' && idType != '' && name != '' && value != '' && 
       desc != '' && setting != '' && schema != ''){
        $.ajax({
            dataType: 'json',
            type:'POST',
            url: url + form_action,
            data:{id:id, idType:idType, name:name, value:value, 
            desc:desc, setting:setting, schema:schema}
        }).done(function(data){
            ...
            $("#create-item").find("textarea[name='schema']").val('');
            getPageData();
            $(".modal").modal('hide');
...
$("body").on("click",".remove-item",function(){
	...
	$.ajax({
		dataType: 'json',
		type:'POST',
		url: url + 'api/delete_tab-enumeration.php',
		data:{id:id}
	}).done(function(data){
		c_obj.remove();
...
$("body").on("click",".edit-item",function(){
    ...
    var id = $(this).parent("td").data('id');
    var idType = $(this).parent("td").prev("td").prev("td").prev("td").prev("td").prev("td").prev("td").text();
    var name = $(this).parent("td").prev("td").prev("td").prev("td").prev("td").prev("td").text();
    var value = $(this).parent("td").prev("td").prev("td").prev("td").prev("td").text();
    var desc = $(this).parent("td").prev("td").prev("td").prev("td").text();
    var setting = $(this).parent("td").prev("td").prev("td").text();
    var schema = $(this).parent("td").prev("td").text();
    ...
    $("#edit-item").find("textarea[name='schema']").val(schema);
...
$(".crud-submit-edit").click(function(e){
    ...
    var schema = $("#edit-item").find("textarea[name='schema']").val();
