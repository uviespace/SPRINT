...
$sql = "SELECT DISTINCT t.* FROM `parameter` t LEFT JOIN `limit` e ON t.id = e.idParameter WHERE  t.idStandard = ".$idStandard." AND e.idParameter IS NOT NULL ORDER BY t.domain, t.name";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<a href='view_parameter-limit.php?idProject=".$idProject."&idStandard=".$idStandard."&idParameter=".$row["id"]."' >".$row["id"]." <b>".$row["domain"]." / ".$row["name"]."</b></a>";
        echo "</div>";
    }
} else {
    echo "0 results";
...
$sql = "SELECT DISTINCT t.* FROM `parameter` t LEFT JOIN `limit` e ON t.id = e.idParameter WHERE  t.idStandard = ".$idStandard." AND e.idParameter IS NULL ORDER BY t.domain, t.name";
