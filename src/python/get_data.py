...
def get_params(db, standard):
    ...
    for row in cur.fetchall():
        ...
        param["ownerStandardId"] = row[1] if row[1] is not None else 0
        if row[2] != None:
...
def get_types(db, standard):
    ...
    for row in cur.fetchall():
        ...
        type_["ownerStandardId"] = row[1] if (row[1] is not None) else 0
        type_["domain"] = row[2]
        type_["name"] = row[3]
...
def get_services(db, standard):
    ...
    for row in cur.fetchall():
        ...
        service["ownerStandardId"] = row[1] if row[1] is not None else 0
        service["name"] = row[2]
        service["desc"] = row[3]
        service["type"] = row[4]
        service["packets"] = []
...
def get_derived_packets(db, parent):
    ...
    for row in cur.fetchall():
        ...
        if (row[4] > 0):             # FIXME: otherwise KeyErr 'kind'
            packet["kind"] = "TM"
        else:
            packet["kind"] = "TC"
...
def get_packets(db, standard):
    ...
    for row in cur.fetchall():
        ...
        packet["ownerStandardId"] = row[1] if row[1] is not None else 0
        packet["spec"] = None
        if row[3] != None:
...
def get_param_sequence(db, standard, _type, packet):
    ...
    offset = 0
    for row in cur.fetchall():
        param = standard["params"]["hash"][row[2]]
        elem = {}        
        elem["id"] = row[0]
        ...
        if offset != None and elem["param"]["_length"] != None:
            offset = offset + elem["param"]["_length"] 
        else:
            offset = None
...
def get_app_standards(db, app):
    cur = db.cursor()
    db_execute(cur, """
        SELECT s.* FROM ApplicationStandard s
        WHERE s.idApplication={0}""".format(str(app["id"])))

    standards = []
    for row in cur.fetchall():
        standard = {}
        standard["standard"] = app["project"]["standards"]["hash"][row[1]]
        standard["relation"] = row[2]
...
def get_app_components(db, app):
    cur = db.cursor()
    db_execute(cur, """
        SELECT s.*, c.* FROM ApplicationComponent s
...
def get_app_specifications(db, app):
    cur = db.cursor()
    db_execute(cur, """
        SELECT ap.*, astd.*, p.* FROM ApplicationPacket ap
        ...
        ORDER BY p.type, p.subtype, p.discriminant""".format(app["id"]))

    specs = []
    for row in cur.fetchall():
        spec = {}        
        spec["app"] = app
        spec["standard"] = app["project"]["standards"]["hash"][row[1]]
        spec["packet"] = spec["standard"]["packets"]["hash"][row[2]]
        spec["relation"] = row[22]
        spec["cmdUsrCheckEnable"] = row[3]
        spec["cmdUsrCheckReady"] = row[4]
        spec["cmdUsrCheckRepeat"] = row[5]
        spec["cmdPrvCheckAcceptance"] = row[6]
        spec["cmdPrvCheckReady"] = row[7]
        spec["cmdUsrActionUpdate"] = row[8]
        spec["cmdPrvActionStart"] = row[9]
        spec["cmdPrvActionProgress"] = row[10]
        spec["cmdPrvActionTermination"] = row[11]
        spec["cmdPrvActionAbort"] = row[12]
        spec["repPrvCheckEnable"] = row[13]
        spec["repPrvCheckReady"] = row[14]
        spec["repPrvCheckRepeat"] = row[15]
        spec["repUsrCheckAcceptance"] = row[16]
        spec["repPrvActionUpdate"] = row[17]
        spec["repUsrActionUpdate"] = row[18]
        specs.append(spec)
