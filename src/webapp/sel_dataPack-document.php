...
<html>
<head>
	<title>Data Package Documents</title>
...
$sql = "SELECT * FROM `projectdatapack` WHERE `idProject` = ".$idProject;
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $sql_docs = "SELECT * FROM `docdatapack` WHERE `idDataPack` = ".$row["id"];
        $result_docs = $mysqli->query($sql_docs);
        $num_rows_docs = mysqli_num_rows($result_docs);
        
        if ($num_rows_docs>0) {

            echo "<div style='height:24px; padding:2px; margin-bottom: 2px; width:50%; background-color:lightblue;'>";
            echo "<a href='view_dataPack-document.php?idProject=".$idProject."&idDataPack=".$row["id"]."' >".$row["id"]." <b>".$row["name"]."</b> (".$num_rows_docs." documents)</a>";
            echo "</div>";
...
<div>
<h2>Data Packages with no Documents</h2>
</div>
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        if ($num_rows_docs==0) {
            ...
            echo "<div style='height:24px; padding:2px; margin-bottom: 2px; width:50%; background-color:lightblue;'>";
            echo "<a href='view_dataPack-document.php?idProject=".$idProject."&idDataPack=".$row["id"]."' >".$row["id"]." <b>".$row["name"]."</b></a>";
            echo "</div>";
