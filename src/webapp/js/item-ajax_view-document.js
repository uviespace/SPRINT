...
function manageData() {
	$.ajax({
		dataType: 'json',
		url: url+'api/getData_view-document.php?idProject='+idProject,
		data: {page:page}
	}).done(function(data){
...
function manageDataAll() {
	...
	$.ajax({
		dataType: 'json',
		url: url+'api/getData_view-document.php?idProject='+idProject+'&showAll=1',
		data: {page:page}
	}).done(function(data){
		total_page = 1;
		current_page = page;
...
function manageRow(data) {
	...
	$.each( data, function( key, value ) {
	  	...
	  	rows = rows + '<td>'+value.idDocType+'</td>';
	  	rows = rows + '<td>'+value.idDocRelation+'</td>';
...
$("body").on("click",".remove-item",function(){
    ...
    if (confirmation) {
        $.ajax({
            dataType: 'json',
            type:'POST',
            url: url + 'api/delete_view-document.php',
            data:{id:id}
        }).done(function(data){
            c_obj.remove();
