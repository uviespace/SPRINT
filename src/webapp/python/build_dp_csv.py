...
def build_dp_csv(app):
    path_base = new_path()
    gen_path(path_base)

    for comp in app["components"]["list"]:
        if comp["shortName"] == "mib":
            #print("found mib")
            break

    cmd = "gen_dp_csv"
    path = "{0}/{1}".format(path_base, "dp_csv")
    gen_path(path)
    try:
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        print("Usage: python build_dp_csv.py {project_id} {application_id}")
